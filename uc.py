import sys
import time
import replit
from termcolor import cprint
import newspaper
ucHashIdentity = ""
def init(GetNewspaper):
	if GetNewspaper:
		paper = newspaper.build('http://cnn.com')
	cprint("Ultracode V3.0 Runtime", 'blue')
	cprint("Initilizing...", 'green')
	time.sleep(1.5)
	cprint("Loading Project...", 'green')
	time.sleep(1.8)
	cprint("Compiling assets...", 'green')
	replit.clear()
	if GetNewspaper:
		return ucHashIdentity, paper
	else:
		return ucHashIdentity
def exit(InitIdentity):
	if InitIdentity == ucHashIdentity:
		sys.exit(0)
	else:
		cprint("Error - Any PureUC project/script must start with the uc.init() method and use the output from that function to call the uc.exit() method.", 'red')
def getArticleSumarryFromPaper(articleNum):
	cnn_paper = newspaper.build('http://cnn.com')
	cnn_article = cnn_paper.articles[articleNum]
	cnn_article.download()
	return cnn_article.sumarry()
def getArticleHTML(CNNarticleNum):
	cnn_paper = newspaper.build('http://cnn.com')
	cnn_article = cnn_paper.articles[CNNarticleNum]
	cnn_article.download()
	return cnn_article.html
def hostArticle(CNNarticleNum):
	from flask import Flask
	app = Flask(__name__)

	@app.route('/')
	def article():
		return getArticleHTML(CNNarticleNum)
	app.run(host='0.0.0.0', port=8080)



def loop(times, run, InitIdentity):
	if InitIdentity == ucHashIdentity:
		for i in range(1, times+1):
			try:
				eval(run)
			except:
				cprint("ERROR - " + Exception + " Please check your code.", 'red')
		i = i + 1
	else:
		cprint("Error - Any PureUC project/script must start with the uc.init() method and use the output from that function to call the uc.loop() method.", 'red')

